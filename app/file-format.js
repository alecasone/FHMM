...
export function encodeWorld(world, history) {
  ...
  const metadata = { version: 3, tileSize: TILE, name: world.name, bounds: world.bounds, sea: world.sea, ocean: world.ocean, biomesVisible: world.biomesVisible, biomeIds: BIOMES.map(b => b.id), rivers: world.rivers, riversVisible: world.riversVisible,
...
function validateMeta(value) {
  if (!value || typeof value !== 'object' || Object.keys(value).some(k => !['bounds', 'sea', 'ocean', 'biomesVisible', 'rivers', 'riversVisible'].includes(k))) throw new Error('Invalid history metadata');
...
export async function decodeWorld(blob) {
  ...
  if (!['FMM1', 'FMM2', MAGIC].includes(magic)) throw new Error('This is not an FMM world file');
  ...
  if (![1, 2, 3].includes(meta.version) || magic !== `FMM${meta.version}` || meta.tileSize !== TILE || !validateBounds(meta.bounds) || !Array.isArray(meta.tiles) || meta.tiles.length > 16000) throw new Error('Unsupported or invalid world file');
