...
export function brushPreviewStamp(mask) {
  ...
  // Only the cursor texture is reduced; terrain painting samples the full mask.
  const width = mask?.width ?? mask?.size ?? 128, height = mask?.height ?? mask?.size ?? 128;
  const size = Math.max(2, Math.min(512, Math.max(width, height)));
  ...
  for (let y = 0; y < size; y++) for (let x = 0; x < size; x++) {
    ...
    const source = mask ? mask.data[Math.round(y / (size - 1) * (height - 1)) * width + Math.round(x / (size - 1) * (width - 1))] / 65535 : 1;
